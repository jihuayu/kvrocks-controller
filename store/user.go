...
func (s *ClusterStore) RemoveUserSessions(ctx context.Context, username string) error {
	entries, err := s.e.List(ctx, sessionPrefix)
	if err != nil {
		return err
	}
	for _, entry := range entries {
		session, err := s.GetSession(ctx, entry.Key)
		if err != nil {
			return err
		}
		...
		if err := s.RemoveSession(ctx, session.ID); err != nil {
			return err
		}
	}
	return nil
